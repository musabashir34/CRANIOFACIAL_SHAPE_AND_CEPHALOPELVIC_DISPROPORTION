---
title: "Postnatal Craniofacial Geometric Morphometrics and Traditional Anthropometry as Markers of Cephalopelvic Disproportion: A Comparative Case-Control Study"
...
make_pub_table <- function(df, footnote_text = NULL, align_cols = NULL, max_width_in = 6.5) {
  ...
  # Size to natural content widths first (fixed layout), then shrink font
  # size as needed so the table never exceeds the printable page width
  # (6.5in on a Letter page with 1in margins)
  ft <- autofit(ft)
...
**Objectives:** Human parturition reflects the “obstetrical dilemma” between fetal cranial form and maternal pelvic constraints. Cephalopelvic disproportion (CPD) prediction relies on pelvimetry and linear fetal head measurements, capturing size but not shape. This study compared anthropometry with geometric morphometric (GM) analysis of neonatal craniofacial shape, both measured postnatally, as retrospective markers of CPD in a Nigerian obstetric population.

**Methods:** In a case-control study, 160 neonates (80 delivered by emergency caesarean section for CPD; 80 uncomplicated vaginal deliveries) were recruited. Standardised anterior (15-landmark) and lateral (14-landmark) photographs were digitised and analysed by Generalised Procrustes Analysis, Principal Component Analysis, Canonical Variate Analysis, Procrustes ANOVA, and MANOVA. Anthropometric variables' joint association with CPD status was assessed by logistic regression model.

**Results:** Occipitofrontal diameter (aOR = 1.29, 95% CI 1.11–1.50, p = 0.0012) and birth weight (aOR = 2.76, 95% CI 1.30–5.84, p = 0.0081) independently predicted CPD; maternal age was protective (aOR = 0.92, 95% CI 0.85–0.99, p = 0.034) (Nagelkerke *R*^2^ = 0.571). After adjustment for maternal covariates and allometry, anterior and lateral craniofacial shape differed significantly between groups (Procrustes ANOVA, F = 5.36, p < 0.001 and F = 3.83, p = 0.0075, respectively).

**Conclusions:** Anthropometry and craniofacial shape both differ significantly between CPD and non-CPD neonates. Allometry test indicates GM shape analysis captures CPD-relevant information not reducible to linear size, consistent with cranio-pelvic morphological integration. Findings are hypothesis-generating rather than a deployable screening tool.

*Keywords: cephalopelvic disproportion; geometric morphometrics; craniofacial shape; Procrustes analysis; obstructed labour; fetal anthropometry*
...
This study compares traditional anthropometric measurements with geometric morphometric analysis of neonatal craniofacial shape, using standardised anterior and lateral photographs obtained after delivery, as retrospective markers of CPD in a case-control sample of neonates delivered at Jahun General Hospital, Jigawa State, Nigeria. We hypothesised that craniofacial shape, independent of size, differs between neonates delivered for CPD and those delivered vaginally without complication, and that this shape difference is not fully explained by differences in fetal size alone. Because both anthropometric and geometric morphometric measurements were obtained postnatally, this study establishes whether such a shape-and-size signal exists and is size-independent, rather than evaluating a tool that could itself be deployed for antenatal risk prediction; whether landmarks equivalent to those used here can be captured antenatally (for example, by three-dimensional or four-dimensional ultrasound) is a separate question requiring dedicated future work.
...
table_presentation <- clinical_table %>%
  mutate(
    Predictor_Variable = case_when(
      ...
      Variable == "`Weight(ibs)`"              ~ "Birth Weight (lbs)",
...
This study demonstrates that both traditional anthropometric measures and geometric morphometric (GM) craniofacial shape features differ significantly between neonates delivered for cephalopelvic disproportion (CPD) and those delivered vaginally without complication. Traditional predictors (specifically occipitofrontal diameter and birth weight) emerged as strong independent risk factors, aligning closely with standard obstetric literature regarding fetal macrosomia and dimensional mismatch [@howells2018predictors; @acog2020macrosomia; @maggy2025computed]. However, the integration of geometric morphometrics expands this perspective by capturing size-independent morphological variations that univariate metrics fail to register.
...
In this Nigerian case-control study, both traditional anthropometric measurements and geometric morphometric craniofacial shape, measured postnatally, differed significantly between neonates delivered for cephalopelvic disproportion and those delivered vaginally without complication. The dissociation between a significant lateral shape difference and a non-significant lateral size difference indicates that craniofacial shape captures CPD-relevant morphological information that is not fully reducible to linear size measurements alone. Ultimately, these findings serve as a proof-of-concept demonstration that a size-independent shape signal exists and is associated with clinical CPD. As all measurements were obtained after delivery, these findings represent retrospective evidence of a CPD-associated craniofacial phenotype rather than a validated antenatal screening tool; translating this shape signal into a prospective, antenatally deployable predictor is an important direction for future research, supporting further research into shape-based approaches to obstetric risk assessment.
...
**Consent for publication:** Mother’s consent was gotten for the images used in Figure 1 and Figure 2 for landmark scheme display
...
**Authors' contributions:** MAB conceived the study, performed the analysis, and drafted the manuscript. IS collected data, digitized the landmarks and revised the manuscript. All authors read and approved the final manuscript.
...
**Artificial Intelligence use:** The authors used Anthropic Claude AI (Sonnet 5) for graphical abstract generation. The authors confirm that they have reviewed and verified the content generated by the AI tool for accuracy and integrity.
